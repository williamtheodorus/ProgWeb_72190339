...
<body>
    ...
    <nav class="navbar navbar-light bg-light">
        <a class="navbar-brand" href="/mahasiswa/edit">Ubah Data Mahasiswa</a>
    </nav>
    <form method="post" action="/mahasiswa/update/{{ $mhs->id }}">
        ...
        @endphp
        <input type="hidden" name="id" id="id" value="@php echo $mhs->id @endphp">
        <div class="form-group">
            <label for="exampleFormControlInput1">NIM</label>
            <input type="number" class="form-control" name="nim" id="nim" value="@php echo $mhs->nim @endphp">
        </div>
        <div class="form-group">
            <label for="exampleFormControlInput1">Nama</label>
            <input type="text" class="form-control" name="nama" id="nama" value="@php echo $mhs->nama @endphp">
        </div>
        <label for="exampleFormControlInput1">Jenis Kelamin</label>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="jenis_kelamin" id="jenis_kelamin" value="Laki-laki"
            @php if (($mhs->jenis_kelamin)=='Laki-laki') echo 'checked' @endphp>
            <label class="form-check-label" for="exampleRadios1">Laki-Laki</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="jenis_kelamin" id="jenis_kelamin" value="Perempuan"
            @php if (($mhs->jenis_kelamin)=='Perempuan') echo 'checked' @endphp>
        ...
        <div class="form-group mt-2">
            ...
            <option value="Informatika"
                @php if (($mhs->jurusan)=='Informatika') echo 'selected' @endphp>Informatika</option>
            <option value="Sistem Informasi"
                @php if (($mhs->jurusan)=='Sistem Informasi') echo 'selected' @endphp>Sistem Informasi</option>
            </select>
        </div>
